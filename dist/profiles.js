...
import { ACCOUNTS_DIR, GEMINI_DIR, fileExists, ensureDir, getOauthCredsPath, getProfileDir, getProfileCredsPath, getCurrentProfilePath, validateProfileName, secureCopyFile, secureWriteFile, safeReadFile, cleanupOldBackups, } from "./utils.js";
...
export async function listProfiles() {
    ...
    for (const entry of entries) {
        if (!entry.isDirectory())
            continue;
        const profileCredsPath = path.join(ACCOUNTS_DIR, entry.name, "oauth_creds.json");
        if (!(await fileExists(profileCredsPath)))
            continue;
...
export async function saveProfile(profileName) {
    ...
    if (!(await fileExists(src))) {
        throw new Error(`No OAuth credentials found at ${src}.\n` +
...
export async function switchProfile(profileName) {
    ...
    if (await fileExists(dest)) {
        ...
        const backupPath = path.join(GEMINI_DIR, `oauth_creds.backup.${timestamp}.json`);
